...
<div class="sort_area">
	...
	<div class="date_view" style="left:39%;">
		<ul>
			...
			<%if(month > 0 ){ %>
			<li><a href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageMonthList.do' />?year=<%=year%>&month=<%=month-1%>" title="이전월으로가기" class="prev">prev</a></li>
			<%}%>
			<li class="date"><%=month+1%> <spring:message code="comCopSmtSim.Navi.Month"/></li><!-- 월 -->
			<%if(month < 11 ){ %>
			<li><a href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageMonthList.do' />?year=<%=year%>&month=<%=month+1%>" class="next" title="다음월으로가기">next</a></li>
			<%}%>
